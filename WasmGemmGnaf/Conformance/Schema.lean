/-
  Schema accounting for the scope-critical definitions.
  ...
  Each exact binding is announced by an `authority-binding:` line carrying the
  exact name the authority JSON uses. An acknowledged mismatch uses
  `authority-gap:`. The checker requires exactly one of those markers per
  authority name and reports gaps as open and uncredited.
...
/--
**The outer shape of `GlobalOptimal` matches the frozen `WGG-GO-1` schema.**
