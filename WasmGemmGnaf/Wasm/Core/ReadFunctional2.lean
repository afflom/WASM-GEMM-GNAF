...
theorem Step_readA.target_functional_group2
    {state : State} {rule : ReadRule} {source left right : List AdminInstr}
    (hgroup : rule.functionalGroup = 2)
  ...
  all_goals try simp_all [constI32_eq_iff, constAddr_eq_iff, Val.toAdmin_eq_iff]
  all_goals
    rcases sourcesEqual with ⟨_, _, hsource⟩
    first
    | exact returnCallRef_source_injective hsource
    | apply returnCallRefFrameAddr_source_injective <;>
        first | assumption | simpa only [List.append_assoc] using hsource
  all_goals try omega
