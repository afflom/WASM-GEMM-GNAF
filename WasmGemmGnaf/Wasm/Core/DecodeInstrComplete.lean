...
theorem flat_op0 {v : Nat} {i : Instr} (hv : v < 0x100) (h : op0 v = some i)
  ...
  rw [decInstrFlat, op0For_pinned, tb_val v hv, h]
...
theorem flat_op1 {v : Nat} {i : Instr} (hv : v < 0x100) (h0 : op0 v = none)
  ...
  rw [decInstrFlat, op0For_pinned, decOp1For_pinned, tb_val v hv, h0]
...
theorem flat_FB {k : Nat} {bo : Bytes} (hbo : Bprefixed 0xFB k bo) (bb r : Bytes)
  ...
  rw [decInstrFlat, op0For_pinned, tb_val 0xFB (by decide),
...
theorem flat_FC {k : Nat} {bo : Bytes} (hbo : Bprefixed 0xFC k bo) (bb r : Bytes)
  ...
  rw [decInstrFlat, op0For_pinned, tb_val 0xFC (by decide),
...
theorem flat_FD {k : Nat} {bo : Bytes} (hbo : Bprefixed 0xFD k bo) (bb r : Bytes)
  ...
  rw [decInstrFlat, op0For_pinned, tb_val 0xFD (by decide),
    show op0 (0xFD : Nat) = none from rfl, if_neg (by decide : ¬ ((0xFD : Nat) = 0xFB)),
    if_neg (by decide : ¬ ((0xFD : Nat) = 0xFC)), if_pos rfl,
    decU32_complete bn x (bb ++ r) hx]
  exact h

/-! The same four entry lemmas for the authority-selected decoder family. -/

theorem flat_op0For [authority : BinaryAuthority]
    {v : Nat} {i : Instr} (hv : v < 0x100) (h : op0 v = some i)
    (hne : v ≠ 0xBB) (r : Bytes) :
    decInstrFlat (tb v :: r) = .ok (i, r) := by
  rw [decInstrFlat, tb_val v hv, op0For_of_ne hne, h]

theorem flat_op1For [authority : BinaryAuthority]
    {v : Nat} {i : Instr} (hv : v < 0x100) (h0 : op0 v = none)
    (hFB : v ≠ 0xFB) (hFC : v ≠ 0xFC) (hFD : v ≠ 0xFD)
    (h14 : v ≠ 0x14) (h15 : v ≠ 0x15)
    (bb r : Bytes) (h : decOp1Base v (bb ++ r) = .ok (i, r)) :
    decInstrFlat (tb v :: (bb ++ r)) = .ok (i, r) := by
  have hBB : v ≠ 0xBB := by
    intro hvBB
    subst v
    cases h0
  rw [decInstrFlat, tb_val v hv, op0For_of_ne hBB, h0]
  simp only [if_neg hFB, if_neg hFC, if_neg hFD,
    decOp1For_of_other h14 h15]
  exact h

theorem flat_FBFor [authority : BinaryAuthority]
    {k : Nat} {bo : Bytes} (hbo : Bprefixed 0xFB k bo) (bb r : Bytes)
    {i : Instr} (h : decFBbody k (bb ++ r) = .ok (i, r)) :
    decInstrFlat (bo ++ (bb ++ r)) = .ok (i, r) := by
  obtain ⟨bn, hbn, x, hx, hxk⟩ := hbo
  subst hbn
  subst hxk
  show decInstrFlat (tb 0xFB :: (bn ++ (bb ++ r))) = _
  rw [decInstrFlat, tb_val 0xFB (by decide), op0For_of_ne (by decide),
    show op0 (0xFB : Nat) = none from rfl, if_pos rfl,
    decU32_complete bn x (bb ++ r) hx]
  exact h

theorem flat_FCFor [authority : BinaryAuthority]
    {k : Nat} {bo : Bytes} (hbo : Bprefixed 0xFC k bo) (bb r : Bytes)
    {i : Instr} (h : decFCbody k (bb ++ r) = .ok (i, r)) :
    decInstrFlat (bo ++ (bb ++ r)) = .ok (i, r) := by
  obtain ⟨bn, hbn, x, hx, hxk⟩ := hbo
  subst hbn
  subst hxk
  show decInstrFlat (tb 0xFC :: (bn ++ (bb ++ r))) = _
  rw [decInstrFlat, tb_val 0xFC (by decide), op0For_of_ne (by decide),
    show op0 (0xFC : Nat) = none from rfl,
    if_neg (by decide : ¬ ((0xFC : Nat) = 0xFB)), if_pos rfl,
    decU32_complete bn x (bb ++ r) hx]
  exact h

theorem flat_FDFor [authority : BinaryAuthority]
    {k : Nat} {bo : Bytes} (hbo : Bprefixed 0xFD k bo) (bb r : Bytes)
    {i : Instr} (h : decFDbody k (bb ++ r) = .ok (i, r)) :
    decInstrFlat (bo ++ (bb ++ r)) = .ok (i, r) := by
  obtain ⟨bn, hbn, x, hx, hxk⟩ := hbo
  subst hbn
  subst hxk
  show decInstrFlat (tb 0xFD :: (bn ++ (bb ++ r))) = _
  rw [decInstrFlat, tb_val 0xFD (by decide), op0For_of_ne (by decide),
    show op0 (0xFD : Nat) = none from rfl,
    if_neg (by decide : ¬ ((0xFD : Nat) = 0xFB)),
    if_neg (by decide : ¬ ((0xFD : Nat) = 0xFC)), if_pos rfl,
    decU32_complete bn x (bb ++ r) hx]
  exact h

/-- Entry through one of the two amended-only one-immediate opcodes. -/
theorem flat_op1Amended {v : Nat} {i : Instr}
    (hv : v < 0x100) (h0 : op0 v = none)
    (hFB : v ≠ 0xFB) (hFC : v ≠ 0xFC) (hFD : v ≠ 0xFD)
    (bb r : Bytes) (h : decOp1' v (bb ++ r) = .ok (i, r)) :
    @decInstrFlat amendedBinaryAuthority (tb v :: (bb ++ r)) = .ok (i, r) := by
  letI : BinaryAuthority := amendedBinaryAuthority
  change decInstrFlat (tb v :: (bb ++ r)) = .ok (i, r)
  have hBB : v ≠ 0xBB := by
    intro hvBB
    subst v
    cases h0
  rw [decInstrFlat, tb_val v hv, op0For_amended, op0'_of_ne hBB, h0]
  simp only [if_neg hFB, if_neg hFC, if_neg hFD, decOp1For_amended]
  exact h
...
theorem flat_BinstrParametric [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrParametric b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | unreachable  =>
      exact flat_op0For (v := 0x00) (by decide) rfl (by decide) r
  | nop  =>
      exact flat_op0For (v := 0x01) (by decide) rfl (by decide) r
  | drop  =>
      exact flat_op0For (v := 0x1A) (by decide) rfl (by decide) r
  | select  =>
      exact flat_op0For (v := 0x1B) (by decide) rfl (by decide) r
  | selectT bs ts hh1 =>
      have h' : decOp1Base 0x1C ((bs) ++ r) = .ok (Instr.select (some ts), r) :=
        arg1_complete (decList_complete decValtype_complete) _ r hh1
      exact flat_op1For (v := 0x1C) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'

/-- Completeness of the flat dispatch on `BinstrControl`: every production of
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrControl [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrControl b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | throw bs x hh1 =>
      have h' : decOp1Base 0x08 ((bs) ++ r) = .ok (Instr.throw x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x08) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | throwRef  =>
      exact flat_op0For (v := 0x0A) (by decide) rfl (by decide) r
  | br bs l hh1 =>
      have h' : decOp1Base 0x0C ((bs) ++ r) = .ok (Instr.br l, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x0C) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | brIf bs l hh1 =>
      have h' : decOp1Base 0x0D ((bs) ++ r) = .ok (Instr.brIf l, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x0D) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | brTable bl bn ls l hh1 hh2 =>
      have h' : decOp1Base 0x0E ((bl ++ bn) ++ r) = .ok (Instr.brTable ls l, r) :=
        arg2_complete (decList_complete decIdx_complete) decIdx_complete _ r hh1 hh2
      exact flat_op1For (v := 0x0E) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bl ++ bn) r h'
  | ret  =>
      exact flat_op0For (v := 0x0F) (by decide) rfl (by decide) r
  | call bs x hh1 =>
      have h' : decOp1Base 0x10 ((bs) ++ r) = .ok (Instr.call x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x10) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | callIndirect by' bx y x hh1 hh2 =>
      have h' : decOp1Base 0x11 ((by' ++ bx) ++ r) = .ok (Instr.callIndirect x (.idx y), r) :=
        arg2_complete decIdx_complete decIdx_complete _ r hh1 hh2
      exact flat_op1For (v := 0x11) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (by' ++ bx) r h'
  | returnCall bs x hh1 =>
      have h' : decOp1Base 0x12 ((bs) ++ r) = .ok (Instr.returnCall x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x12) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | returnCallIndirect by' bx y x hh1 hh2 =>
      have h' : decOp1Base 0x13 ((by' ++ bx) ++ r) = .ok (Instr.returnCallIndirect x (.idx y), r) :=
        arg2_complete decIdx_complete decIdx_complete _ r hh1 hh2
      exact flat_op1For (v := 0x13) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (by' ++ bx) r h'

/-- Completeness of the selected control fragment, including AMD-010's two
typed-reference call productions in the amended instance. -/
theorem flat_BinstrControlFor [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrControlFor b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases authority with
  | mk revision =>
      cases revision with
      | pinned =>
          change BinstrControl b i at h
          exact flat_BinstrControl h r
      | amended =>
          change BinstrControl' b i at h
          cases h with
          | ofPinned _ _ hp =>
              exact @flat_BinstrControl amendedBinaryAuthority b i hp r
          | callRef bs x hx =>
              have hd := decOp1'_callRef_complete (r := r) hx
              exact flat_op1Amended (v := 0x14) (by decide) rfl
                (by decide) (by decide) (by decide) bs r hd
          | returnCallRef bs x hx =>
              have hd := decOp1'_returnCallRef_complete (r := r) hx
              exact flat_op1Amended (v := 0x15) (by decide) rfl
                (by decide) (by decide) (by decide) bs r hd

/-- Completeness of the flat dispatch on `BinstrLocal`: every production of
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrLocal [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrLocal b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | get bs x hh1 =>
      have h' : decOp1Base 0x20 ((bs) ++ r) = .ok (Instr.localGet x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x20) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | set bs x hh1 =>
      have h' : decOp1Base 0x21 ((bs) ++ r) = .ok (Instr.localSet x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x21) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | tee bs x hh1 =>
      have h' : decOp1Base 0x22 ((bs) ++ r) = .ok (Instr.localTee x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x22) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'

/-- Completeness of the flat dispatch on `BinstrGlobal`: every production of
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrGlobal [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrGlobal b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | get bs x hh1 =>
      have h' : decOp1Base 0x23 ((bs) ++ r) = .ok (Instr.globalGet x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x23) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | set bs x hh1 =>
      have h' : decOp1Base 0x24 ((bs) ++ r) = .ok (Instr.globalSet x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x24) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'

/-- Completeness of the flat dispatch on `BinstrTable`: every production of
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrTable [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrTable b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | get bs x hh1 =>
      have h' : decOp1Base 0x25 ((bs) ++ r) = .ok (Instr.tableGet x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x25) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | set bs x hh1 =>
      have h' : decOp1Base 0x26 ((bs) ++ r) = .ok (Instr.tableSet x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x26) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrMemory [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrMemory b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | i32Load bm x ao hh1 =>
      have h' : decOp1Base 0x28 ((bm) ++ r) = .ok (Instr.load .i32 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x28) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Load bm x ao hh1 =>
      have h' : decOp1Base 0x29 ((bm) ++ r) = .ok (Instr.load .i64 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x29) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | f32Load bm x ao hh1 =>
      have h' : decOp1Base 0x2A ((bm) ++ r) = .ok (Instr.load .f32 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x2A) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | f64Load bm x ao hh1 =>
      have h' : decOp1Base 0x2B ((bm) ++ r) = .ok (Instr.load .f64 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x2B) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i32Load8S bm x ao hh1 =>
      have h' : decOp1Base 0x2C ((bm) ++ r) = .ok (Instr.load .i32 (some { sz := .s8, sx := .s }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x2C) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i32Load8U bm x ao hh1 =>
      have h' : decOp1Base 0x2D ((bm) ++ r) = .ok (Instr.load .i32 (some { sz := .s8, sx := .u }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x2D) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i32Load16S bm x ao hh1 =>
      have h' : decOp1Base 0x2E ((bm) ++ r) = .ok (Instr.load .i32 (some { sz := .s16, sx := .s }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x2E) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i32Load16U bm x ao hh1 =>
      have h' : decOp1Base 0x2F ((bm) ++ r) = .ok (Instr.load .i32 (some { sz := .s16, sx := .u }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x2F) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Load8S bm x ao hh1 =>
      have h' : decOp1Base 0x30 ((bm) ++ r) = .ok (Instr.load .i64 (some { sz := .s8, sx := .s }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x30) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Load8U bm x ao hh1 =>
      have h' : decOp1Base 0x31 ((bm) ++ r) = .ok (Instr.load .i64 (some { sz := .s8, sx := .u }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x31) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Load16S bm x ao hh1 =>
      have h' : decOp1Base 0x32 ((bm) ++ r) = .ok (Instr.load .i64 (some { sz := .s16, sx := .s }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x32) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Load16U bm x ao hh1 =>
      have h' : decOp1Base 0x33 ((bm) ++ r) = .ok (Instr.load .i64 (some { sz := .s16, sx := .u }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x33) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Load32S bm x ao hh1 =>
      have h' : decOp1Base 0x34 ((bm) ++ r) = .ok (Instr.load .i64 (some { sz := .s32, sx := .s }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x34) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Load32U bm x ao hh1 =>
      have h' : decOp1Base 0x35 ((bm) ++ r) = .ok (Instr.load .i64 (some { sz := .s32, sx := .u }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x35) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i32Store bm x ao hh1 =>
      have h' : decOp1Base 0x36 ((bm) ++ r) = .ok (Instr.store .i32 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x36) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Store bm x ao hh1 =>
      have h' : decOp1Base 0x37 ((bm) ++ r) = .ok (Instr.store .i64 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x37) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | f32Store bm x ao hh1 =>
      have h' : decOp1Base 0x38 ((bm) ++ r) = .ok (Instr.store .f32 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x38) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | f64Store bm x ao hh1 =>
      have h' : decOp1Base 0x39 ((bm) ++ r) = .ok (Instr.store .f64 none x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x39) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i32Store8 bm x ao hh1 =>
      have h' : decOp1Base 0x3A ((bm) ++ r) = .ok (Instr.store .i32 (some { sz := .s8 }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x3A) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i32Store16 bm x ao hh1 =>
      have h' : decOp1Base 0x3B ((bm) ++ r) = .ok (Instr.store .i32 (some { sz := .s16 }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x3B) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Store8 bm x ao hh1 =>
      have h' : decOp1Base 0x3C ((bm) ++ r) = .ok (Instr.store .i64 (some { sz := .s8 }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x3C) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Store16 bm x ao hh1 =>
      have h' : decOp1Base 0x3D ((bm) ++ r) = .ok (Instr.store .i64 (some { sz := .s16 }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x3D) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | i64Store32 bm x ao hh1 =>
      have h' : decOp1Base 0x3E ((bm) ++ r) = .ok (Instr.store .i64 (some { sz := .s32 }) x ao, r) :=
        arg1_complete decMemarg_complete _ r hh1
      exact flat_op1For (v := 0x3E) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bm) r h'
  | size bs x hh1 =>
      have h' : decOp1Base 0x3F ((bs) ++ r) = .ok (Instr.memorySize x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x3F) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | grow bs x hh1 =>
      have h' : decOp1Base 0x40 ((bs) ++ r) = .ok (Instr.memoryGrow x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0x40) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | init bo by' bx y x hbo hh1 hh2 =>
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrRef [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrRef b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | null bs ht hh1 =>
      have h' : decOp1Base 0xD0 ((bs) ++ r) = .ok (Instr.refNull ht, r) :=
        arg1_complete decHeaptype_complete _ r hh1
      exact flat_op1For (v := 0xD0) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | isNull  =>
      exact flat_op0For (v := 0xD1) (by decide) rfl (by decide) r
  | func bs x hh1 =>
      have h' : decOp1Base 0xD2 ((bs) ++ r) = .ok (Instr.refFunc x, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0xD2) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | eq  =>
      exact flat_op0For (v := 0xD3) (by decide) rfl (by decide) r
  | asNonNull  =>
      exact flat_op0For (v := 0xD4) (by decide) rfl (by decide) r
  | brOnNull bs l hh1 =>
      have h' : decOp1Base 0xD5 ((bs) ++ r) = .ok (Instr.brOnNull l, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0xD5) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | brOnNonNull bs l hh1 =>
      have h' : decOp1Base 0xD6 ((bs) ++ r) = .ok (Instr.brOnNonNull l, r) :=
        arg1_complete decIdx_complete _ r hh1
      exact flat_op1For (v := 0xD6) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'

/-- Completeness of the flat dispatch on `BinstrStruct`: every production of
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrStruct [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrStruct b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | new bo bx x hbo hh1 =>
  ...
  | get bo bx bi x i hbo hh1 hh2 =>
      ...
      exact flat_FBFor hbo (bx ++ bi) r h'
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrArray [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrArray b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | new bo bx x hbo hh1 =>
  ...
  | newFixed bo bx bn x n hbo hh1 hh2 =>
      ...
      exact flat_FBFor hbo (bx ++ bn) r h'
  | newData bo bx by' x y hbo hh1 hh2 =>
      ...
      exact flat_FBFor hbo (bx ++ by') r h'
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrCast [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrCast b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  ...
  | brOnCast bo bc bl b₁ b₂ n₁ n₂ l ht₁ ht₂ hbo hh1 hh2 hh3 hh4 =>
      ...
      exact flat_FBFor hbo (bc ++ bl ++ b₁ ++ b₂) r h'
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrExtern [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrExtern b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | anyConvertExtern =>
      rename_i hbo
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrI31 [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrI31 b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | refI31 =>
      rename_i hbo
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrNum [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrNum b i) (r : Bytes)
    (hsafe : authority.revision = .amended → i ≠ pinnedBadPromote) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | i32Const bs n hh1 =>
      have h' : decOp1Base 0x41 ((bs) ++ r) = .ok (Instr.const .i32 n, r) :=
        arg1_complete decU32_complete _ r hh1
      exact flat_op1For (v := 0x41) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | i64Const bs n hh1 =>
      have h' : decOp1Base 0x42 ((bs) ++ r) = .ok (Instr.const .i64 n, r) :=
        arg1_complete decU64_complete _ r hh1
      exact flat_op1For (v := 0x42) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | f32Const bs p hh1 =>
      have h' : decOp1Base 0x43 ((bs) ++ r) = .ok (Instr.const .f32 p, r) :=
        arg1_complete decF32_complete _ r hh1
      exact flat_op1For (v := 0x43) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | f64Const bs p hh1 =>
      have h' : decOp1Base 0x44 ((bs) ++ r) = .ok (Instr.const .f64 p, r) :=
        arg1_complete decF64_complete _ r hh1
      exact flat_op1For (v := 0x44) (by decide) rfl (by decide) (by decide) (by decide) (by decide) (by decide)
        (bs) r h'
  | i32Eqz  =>
      exact flat_op0For (v := 0x45) (by decide) rfl (by decide) r
  | i32Eq  =>
      exact flat_op0For (v := 0x46) (by decide) rfl (by decide) r
  | i32Ne  =>
      exact flat_op0For (v := 0x47) (by decide) rfl (by decide) r
  | i32LtS  =>
      exact flat_op0For (v := 0x48) (by decide) rfl (by decide) r
  | i32LtU  =>
      exact flat_op0For (v := 0x49) (by decide) rfl (by decide) r
  | i32GtS  =>
      exact flat_op0For (v := 0x4A) (by decide) rfl (by decide) r
  | i32GtU  =>
      exact flat_op0For (v := 0x4B) (by decide) rfl (by decide) r
  | i32LeS  =>
      exact flat_op0For (v := 0x4C) (by decide) rfl (by decide) r
  | i32LeU  =>
      exact flat_op0For (v := 0x4D) (by decide) rfl (by decide) r
  | i32GeS  =>
      exact flat_op0For (v := 0x4E) (by decide) rfl (by decide) r
  | i32GeU  =>
      exact flat_op0For (v := 0x4F) (by decide) rfl (by decide) r
  | i64Eqz  =>
      exact flat_op0For (v := 0x50) (by decide) rfl (by decide) r
  | i64Eq  =>
      exact flat_op0For (v := 0x51) (by decide) rfl (by decide) r
  | i64Ne  =>
      exact flat_op0For (v := 0x52) (by decide) rfl (by decide) r
  | i64LtS  =>
      exact flat_op0For (v := 0x53) (by decide) rfl (by decide) r
  | i64LtU  =>
      exact flat_op0For (v := 0x54) (by decide) rfl (by decide) r
  | i64GtS  =>
      exact flat_op0For (v := 0x55) (by decide) rfl (by decide) r
  | i64GtU  =>
      exact flat_op0For (v := 0x56) (by decide) rfl (by decide) r
  | i64LeS  =>
      exact flat_op0For (v := 0x57) (by decide) rfl (by decide) r
  | i64LeU  =>
      exact flat_op0For (v := 0x58) (by decide) rfl (by decide) r
  | i64GeS  =>
      exact flat_op0For (v := 0x59) (by decide) rfl (by decide) r
  | i64GeU  =>
      exact flat_op0For (v := 0x5A) (by decide) rfl (by decide) r
  | f32Eq  =>
      exact flat_op0For (v := 0x5B) (by decide) rfl (by decide) r
  | f32Ne  =>
      exact flat_op0For (v := 0x5C) (by decide) rfl (by decide) r
  | f32Lt  =>
      exact flat_op0For (v := 0x5D) (by decide) rfl (by decide) r
  | f32Gt  =>
      exact flat_op0For (v := 0x5E) (by decide) rfl (by decide) r
  | f32Le  =>
      exact flat_op0For (v := 0x5F) (by decide) rfl (by decide) r
  | f32Ge  =>
      exact flat_op0For (v := 0x60) (by decide) rfl (by decide) r
  | f64Eq  =>
      exact flat_op0For (v := 0x61) (by decide) rfl (by decide) r
  | f64Ne  =>
      exact flat_op0For (v := 0x62) (by decide) rfl (by decide) r
  | f64Lt  =>
      exact flat_op0For (v := 0x63) (by decide) rfl (by decide) r
  | f64Gt  =>
      exact flat_op0For (v := 0x64) (by decide) rfl (by decide) r
  | f64Le  =>
      exact flat_op0For (v := 0x65) (by decide) rfl (by decide) r
  | f64Ge  =>
      exact flat_op0For (v := 0x66) (by decide) rfl (by decide) r
  | i32Clz  =>
      exact flat_op0For (v := 0x67) (by decide) rfl (by decide) r
  | i32Ctz  =>
      exact flat_op0For (v := 0x68) (by decide) rfl (by decide) r
  | i32Popcnt  =>
      exact flat_op0For (v := 0x69) (by decide) rfl (by decide) r
  | i32Add  =>
      exact flat_op0For (v := 0x6A) (by decide) rfl (by decide) r
  | i32Sub  =>
      exact flat_op0For (v := 0x6B) (by decide) rfl (by decide) r
  | i32Mul  =>
      exact flat_op0For (v := 0x6C) (by decide) rfl (by decide) r
  | i32DivS  =>
      exact flat_op0For (v := 0x6D) (by decide) rfl (by decide) r
  | i32DivU  =>
      exact flat_op0For (v := 0x6E) (by decide) rfl (by decide) r
  | i32RemS  =>
      exact flat_op0For (v := 0x6F) (by decide) rfl (by decide) r
  | i32RemU  =>
      exact flat_op0For (v := 0x70) (by decide) rfl (by decide) r
  | i32And  =>
      exact flat_op0For (v := 0x71) (by decide) rfl (by decide) r
  | i32Or  =>
      exact flat_op0For (v := 0x72) (by decide) rfl (by decide) r
  | i32Xor  =>
      exact flat_op0For (v := 0x73) (by decide) rfl (by decide) r
  | i32Shl  =>
      exact flat_op0For (v := 0x74) (by decide) rfl (by decide) r
  | i32ShrS  =>
      exact flat_op0For (v := 0x75) (by decide) rfl (by decide) r
  | i32ShrU  =>
      exact flat_op0For (v := 0x76) (by decide) rfl (by decide) r
  | i32Rotl  =>
      exact flat_op0For (v := 0x77) (by decide) rfl (by decide) r
  | i32Rotr  =>
      exact flat_op0For (v := 0x78) (by decide) rfl (by decide) r
  | i64Clz  =>
      exact flat_op0For (v := 0x79) (by decide) rfl (by decide) r
  | i64Ctz  =>
      exact flat_op0For (v := 0x7A) (by decide) rfl (by decide) r
  | i64Popcnt  =>
      exact flat_op0For (v := 0x7B) (by decide) rfl (by decide) r
  | i32Extend8  =>
      exact flat_op0For (v := 0xC0) (by decide) rfl (by decide) r
  | i32Extend16  =>
      exact flat_op0For (v := 0xC1) (by decide) rfl (by decide) r
  | i64Extend8  =>
      exact flat_op0For (v := 0xC2) (by decide) rfl (by decide) r
  | i64Extend16  =>
      exact flat_op0For (v := 0xC3) (by decide) rfl (by decide) r
  | i64Extend32  =>
      exact flat_op0For (v := 0xC4) (by decide) rfl (by decide) r
  | i64Add  =>
      exact flat_op0For (v := 0x7C) (by decide) rfl (by decide) r
  | i64Sub  =>
      exact flat_op0For (v := 0x7D) (by decide) rfl (by decide) r
  | i64Mul  =>
      exact flat_op0For (v := 0x7E) (by decide) rfl (by decide) r
  | i64DivS  =>
      exact flat_op0For (v := 0x7F) (by decide) rfl (by decide) r
  | i64DivU  =>
      exact flat_op0For (v := 0x80) (by decide) rfl (by decide) r
  | i64RemS  =>
      exact flat_op0For (v := 0x81) (by decide) rfl (by decide) r
  | i64RemU  =>
      exact flat_op0For (v := 0x82) (by decide) rfl (by decide) r
  | i64And  =>
      exact flat_op0For (v := 0x83) (by decide) rfl (by decide) r
  | i64Or  =>
      exact flat_op0For (v := 0x84) (by decide) rfl (by decide) r
  | i64Xor  =>
      exact flat_op0For (v := 0x85) (by decide) rfl (by decide) r
  | i64Shl  =>
      exact flat_op0For (v := 0x86) (by decide) rfl (by decide) r
  | i64ShrS  =>
      exact flat_op0For (v := 0x87) (by decide) rfl (by decide) r
  | i64ShrU  =>
      exact flat_op0For (v := 0x88) (by decide) rfl (by decide) r
  | i64Rotl  =>
      exact flat_op0For (v := 0x89) (by decide) rfl (by decide) r
  | i64Rotr  =>
      exact flat_op0For (v := 0x8A) (by decide) rfl (by decide) r
  | f32Abs  =>
      exact flat_op0For (v := 0x8B) (by decide) rfl (by decide) r
  | f32Neg  =>
      exact flat_op0For (v := 0x8C) (by decide) rfl (by decide) r
  | f32Ceil  =>
      exact flat_op0For (v := 0x8D) (by decide) rfl (by decide) r
  | f32Floor  =>
      exact flat_op0For (v := 0x8E) (by decide) rfl (by decide) r
  | f32Trunc  =>
      exact flat_op0For (v := 0x8F) (by decide) rfl (by decide) r
  | f32Nearest  =>
      exact flat_op0For (v := 0x90) (by decide) rfl (by decide) r
  | f32Sqrt  =>
      exact flat_op0For (v := 0x91) (by decide) rfl (by decide) r
  | f32Add  =>
      exact flat_op0For (v := 0x92) (by decide) rfl (by decide) r
  | f32Sub  =>
      exact flat_op0For (v := 0x93) (by decide) rfl (by decide) r
  | f32Mul  =>
      exact flat_op0For (v := 0x94) (by decide) rfl (by decide) r
  | f32Div  =>
      exact flat_op0For (v := 0x95) (by decide) rfl (by decide) r
  | f32Min  =>
      exact flat_op0For (v := 0x96) (by decide) rfl (by decide) r
  | f32Max  =>
      exact flat_op0For (v := 0x97) (by decide) rfl (by decide) r
  | f32Copysign  =>
      exact flat_op0For (v := 0x98) (by decide) rfl (by decide) r
  | f64Abs  =>
      exact flat_op0For (v := 0x99) (by decide) rfl (by decide) r
  | f64Neg  =>
      exact flat_op0For (v := 0x9A) (by decide) rfl (by decide) r
  | f64Ceil  =>
      exact flat_op0For (v := 0x9B) (by decide) rfl (by decide) r
  | f64Floor  =>
      exact flat_op0For (v := 0x9C) (by decide) rfl (by decide) r
  | f64Trunc  =>
      exact flat_op0For (v := 0x9D) (by decide) rfl (by decide) r
  | f64Nearest  =>
      exact flat_op0For (v := 0x9E) (by decide) rfl (by decide) r
  | f64Sqrt  =>
      exact flat_op0For (v := 0x9F) (by decide) rfl (by decide) r
  | f64Add  =>
      exact flat_op0For (v := 0xA0) (by decide) rfl (by decide) r
  | f64Sub  =>
      exact flat_op0For (v := 0xA1) (by decide) rfl (by decide) r
  | f64Mul  =>
      exact flat_op0For (v := 0xA2) (by decide) rfl (by decide) r
  | f64Div  =>
      exact flat_op0For (v := 0xA3) (by decide) rfl (by decide) r
  | f64Min  =>
      exact flat_op0For (v := 0xA4) (by decide) rfl (by decide) r
  | f64Max  =>
      exact flat_op0For (v := 0xA5) (by decide) rfl (by decide) r
  | f64Copysign  =>
      exact flat_op0For (v := 0xA6) (by decide) rfl (by decide) r
  | i32WrapI64  =>
      exact flat_op0For (v := 0xA7) (by decide) rfl (by decide) r
  | i32TruncF32S  =>
      exact flat_op0For (v := 0xA8) (by decide) rfl (by decide) r
  | i32TruncF32U  =>
      exact flat_op0For (v := 0xA9) (by decide) rfl (by decide) r
  | i32TruncF64S  =>
      exact flat_op0For (v := 0xAA) (by decide) rfl (by decide) r
  | i32TruncF64U  =>
      exact flat_op0For (v := 0xAB) (by decide) rfl (by decide) r
  | i64ExtendI32S  =>
      exact flat_op0For (v := 0xAC) (by decide) rfl (by decide) r
  | i64ExtendI32U  =>
      exact flat_op0For (v := 0xAD) (by decide) rfl (by decide) r
  | i64TruncF32S  =>
      exact flat_op0For (v := 0xAE) (by decide) rfl (by decide) r
  | i64TruncF32U  =>
      exact flat_op0For (v := 0xAF) (by decide) rfl (by decide) r
  | i64TruncF64S  =>
      exact flat_op0For (v := 0xB0) (by decide) rfl (by decide) r
  | i64TruncF64U  =>
      exact flat_op0For (v := 0xB1) (by decide) rfl (by decide) r
  | f32ConvertI32S  =>
      exact flat_op0For (v := 0xB2) (by decide) rfl (by decide) r
  | f32ConvertI32U  =>
      exact flat_op0For (v := 0xB3) (by decide) rfl (by decide) r
  | f32ConvertI64S  =>
      exact flat_op0For (v := 0xB4) (by decide) rfl (by decide) r
  | f32ConvertI64U  =>
      exact flat_op0For (v := 0xB5) (by decide) rfl (by decide) r
  | f32DemoteF64  =>
      exact flat_op0For (v := 0xB6) (by decide) rfl (by decide) r
  | f64ConvertI32S  =>
      exact flat_op0For (v := 0xB7) (by decide) rfl (by decide) r
  | f64ConvertI32U  =>
      exact flat_op0For (v := 0xB8) (by decide) rfl (by decide) r
  | f64ConvertI64S  =>
      exact flat_op0For (v := 0xB9) (by decide) rfl (by decide) r
  | f64ConvertI64U  =>
      exact flat_op0For (v := 0xBA) (by decide) rfl (by decide) r
  | f32PromoteF64  =>
      cases authority with
      | mk revision =>
          cases revision with
          | pinned => exact flat_op0 (v := 0xBB) (by decide) rfl r
          | amended => exact (hsafe rfl rfl).elim
  | i32ReinterpretF32  =>
      exact flat_op0For (v := 0xBC) (by decide) rfl (by decide) r
  | i64ReinterpretF64  =>
      exact flat_op0For (v := 0xBD) (by decide) rfl (by decide) r
  | f32ReinterpretI32  =>
      exact flat_op0For (v := 0xBE) (by decide) rfl (by decide) r
  | f64ReinterpretI64  =>
      exact flat_op0For (v := 0xBF) (by decide) rfl (by decide) r
  | i32TruncSatF32S =>
      rename_i hbo
      exact flat_FCFor hbo [] r rfl
  | i32TruncSatF32U =>
      rename_i hbo
      exact flat_FCFor hbo [] r rfl
  | i32TruncSatF64S =>
...
/-- Completeness of the selected numeric fragment, excluding the malformed
pinned 0xBB result exactly when the amended relation replaces it. -/
theorem flat_BinstrNumFor [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrNumFor b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases authority with
  | mk revision =>
      cases revision with
      | pinned =>
          change BinstrNum b i at h
          exact flat_BinstrNum h r (by intro ha; cases ha)
      | amended =>
          change BinstrNum' b i at h
          cases h with
          | ofPinned _ _ hp hne =>
              exact @flat_BinstrNum amendedBinaryAuthority b i hp r
                (fun _ => hne)
          | f64PromoteF32 =>
              letI : BinaryAuthority := amendedBinaryAuthority
              change decInstrFlat (tb 0xBB :: r) =
                .ok (.cvtop .f64 .f32 (.ff .promote), r)
              rw [decInstrFlat, tb_val 0xBB (by decide), op0For_amended,
                op0'_promote]

/-- Completeness of the flat dispatch on `BinstrVecMem`: every production of
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrVecMem [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrVecMem b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  ...
  | v128Load8Lane bo bm bi x ao i hbo hh1 hh2 =>
      ...
      exact flat_FDFor hbo (bm ++ bi) r h'
  ...
  | i8x16Shuffle bo bl ls hbo hrep =>
      ...
      exact flat_FDFor hbo bl r h'
  | i8x16Swizzle =>
      rename_i hbo
      exact flat_FDFor hbo [] r rfl
  | i8x16RelaxedSwizzle =>
      rename_i hbo
      exact flat_FDFor hbo [] r rfl
  | i8x16Splat =>
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrVecRel [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrVecRel b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | i8x16Eq =>
      rename_i hbo
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrVecV128 [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrVecV128 b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | not =>
      rename_i hbo
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrVecInt8And16 [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrVecInt8And16 b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | i8x16Abs =>
      rename_i hbo
...
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrVecInt32And64_except_bad [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrVecInt32And64 b i)
    (hne : i ≠ pinnedBadRelaxedDotAdd) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
...
/-- Completeness of the authority-selected AMD-012 vector leaf. -/
theorem flat_BinstrVecInt32And64For [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrVecInt32And64For b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases authority with
  | mk revision =>
      cases revision with
      | pinned =>
          change BinstrVecInt32And64 b i at h
          by_cases hi : i = pinnedBadRelaxedDotAdd
          · subst i
            have hbo : Bprefixed 0xFD 275 b := by
              cases h
              assumption
            exact flat_FDFor hbo [] r rfl
          · exact @flat_BinstrVecInt32And64_except_bad
              pinnedBinaryAuthority b i h hi r
      | amended =>
          change BinstrVecInt32And64' b i at h
          cases h with
          | ofPinned _ hp hne =>
              exact @flat_BinstrVecInt32And64_except_bad
                amendedBinaryAuthority b i hp hne r
          | correctedRelaxedDotAdd hbo =>
              exact @flat_FDFor amendedBinaryAuthority 275 b hbo [] r _ rfl

/-- Completeness of the flat dispatch on `BinstrVecFloat`: every production of
this fragment of the pinned grammar is decoded, to the value the grammar
gives it, consuming exactly the derivation's bytes. -/
theorem flat_BinstrVecFloat [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : BinstrVecFloat b i) (r : Bytes) :
    decInstrFlat (b ++ r) = .ok (i, r) := by
  cases h with
  | f32x4Ceil =>
      rename_i hbo
...
theorem Binstr_ne_nil [authority : BinaryAuthority]
    {b : Bytes} {i : Instr} (h : Binstr b i) : b ≠ [] := by
  cases h with
  | block => simp
  | loop => simp
  | ifThen => simp
  ...
  | ofControl =>
      rename_i h'; intro hb
      have hx := flat_BinstrControlFor h' ([] : Bytes); rw [hb] at hx
      simp [decInstrFlat] at hx
  | ofLocal =>
      rename_i h'; intro hb
  ...
  | ofNum =>
      rename_i h'; intro hb
      have hx := flat_BinstrNumFor h' ([] : Bytes); rw [hb] at hx
      simp [decInstrFlat] at hx
  | ofVecMem =>
      rename_i h'; intro hb
  ...
  | ofVecInt32And64 =>
      rename_i h'; intro hb
      have hx := flat_BinstrVecInt32And64For h' ([] : Bytes); rw [hb] at hx
      simp [decInstrFlat] at hx
  | ofVecFloat =>
      rename_i h'; intro hb
...
theorem Binstrs_length_le_aux [authority : BinaryAuthority] :
    ∀ (n : Nat) (b : Bytes) (is : List Instr),
...
theorem Binstrs_length_le [authority : BinaryAuthority]
    {b : Bytes} {is : List Instr} (h : Binstrs b is) :
    is.length ≤ b.length :=
  Binstrs_length_le_aux b.length b is (Nat.le_refl _) h

/-! ## `END` and `ELSE` are not opcodes -/

theorem decInstrFlat_error [authority : BinaryAuthority]
    {c : Byte} {t : Bytes} {v : Nat} (hv : c.val = v)
    (h0 : op0 v = none) (h1 : ¬ (v = 0xFB)) (h2 : ¬ (v = 0xFC)) (h3 : ¬ (v = 0xFD))
    (hBB : v ≠ 0xBB) (h14 : v ≠ 0x14) (h15 : v ≠ 0x15)
    (h4 : decOp1Base v t = .error .opcode) :
    decInstrFlat (c :: t) = .error .opcode := by
  rw [decInstrFlat, hv, op0For_of_ne hBB, h0]
  simp only [if_neg h1, if_neg h2, if_neg h3,
    decOp1For_of_other h14 h15]
  exact h4
...
theorem decInstrFlat_head [authority : BinaryAuthority]
    {c : Byte} {t : Bytes} {i : Instr} {r : Bytes}
    (h : decInstrFlat (c :: t) = .ok (i, r)) :
    ¬ (c.val = 0x02) ∧ ¬ (c.val = 0x03) ∧ ¬ (c.val = 0x04) ∧ ¬ (c.val = 0x1F) := by
  refine ⟨?_, ?_, ?_, ?_⟩ <;> intro hc
  · rw [decInstrFlat_error hc rfl (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) rfl] at h; cases h
  · rw [decInstrFlat_error hc rfl (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) rfl] at h; cases h
  · rw [decInstrFlat_error hc rfl (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) rfl] at h; cases h
  · rw [decInstrFlat_error hc rfl (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) rfl] at h; cases h

theorem decInstr_of_flat [authority : BinaryAuthority]
    {d : Nat} {c : Byte} {t : Bytes} {i : Instr} {r : Bytes}
...
theorem decInstr_ne_end [authority : BinaryAuthority]
    {d : Nat} {c : Byte} {t : Bytes} (hb : c.val = 0x0B ∨ c.val = 0x05)
  ...
  | succ d =>
      ...
      have hflat : decInstrFlat (c :: t) = .error .opcode := by
        rcases hb with h | h
        · exact decInstrFlat_error h rfl (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) rfl
        · exact decInstrFlat_error h rfl (by decide) (by decide) (by decide) (by decide) (by decide) (by decide) rfl
      rw [decInstr]
...
theorem decInstrs_complete_of [authority : BinaryAuthority] (d M : Nat)
...
theorem decInstr_complete [authority : BinaryAuthority] :
    ∀ (N d : Nat) (b : Bytes) (i : Instr) (r : Bytes),
  ...
  | succ N ihN =>
      ...
      | ofControl =>
          rename_i h'
          have hf := flat_BinstrControlFor h' r
      ...
      | ofVecInt32And64 =>
          rename_i h'
          have hf := flat_BinstrVecInt32And64For h' r
...
theorem decInstr_completeD [authority : BinaryAuthority]
    (d : Nat) (b : Bytes) (i : Instr) (r : Bytes)
...
decoded exactly. -/
theorem decExpr_completeD [authority : BinaryAuthority]
    (d : Nat) (b : Bytes) (e : Expr) (r : Bytes)
...
theorem decInstrA_completeD (d : Nat) (b : Bytes) (i : Instr) (r : Bytes)
    (hd : b.length ≤ d) (h : BinstrA b i) :
    decInstrA d (b ++ r) = .ok (i, r) :=
  @decInstr_completeD amendedBinaryAuthority d b i r hd h

theorem decExprA_completeD (d : Nat) (b : Bytes) (e : Expr) (r : Bytes)
    (hd : b.length ≤ d) (h : BexprA b e) :
    decExprA d (b ++ r) = .ok (e, r) :=
  @decExpr_completeD amendedBinaryAuthority d b e r hd h
